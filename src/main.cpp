...
bool CheckInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck>* pvChecks)
{
    if (!tx.IsCoinBase()) {
        ...
        if (!tx.IsCoinStake() && pindexPrev->nHeight > 176891) {
                if (nValueIn < tx.GetValueOut() && pindexPrev->nHeight > 176891)
                ...
                if (nTxFee < 0 && pindexPrev->nHeight > 176891)
                    return state.DoS(100, error("CheckInputs() : %s nTxFee < 0", tx.GetHash().ToString()),
                        REJECT_INVALID, "bad-txns-fee-negative");
                nFees += nTxFee;
                if (!MoneyRange(nFees) && pindexPrev->nHeight > 176891)
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.IsProofOfStake() && (pindexPrev->nHeight > 306833) && !IsInitialBlockDownload()) {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake() && (pindex->nHeight > 306833) && !IsInitialBlockDownload()) {
        LOCK(cs_main);
